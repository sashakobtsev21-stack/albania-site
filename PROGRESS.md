...
| R0.5 Стратегия | ✅ | docs/STRATEGY.md (EN-first, гео, моат) |
...
| R3 Контент | 🟡 | 5 статей (cover-only) + столица Тирана (18 фото, Нед.1) + город Саранда (14 фото, Нед.2) + город Берат (12 фото, Нед.3) + город Гирокастра (13 фото, Нед.6) + МАРШРУТ Albania Road Trip (6 остановок, 10 фото, Нед.3) + статья «Is Albania Safe to Visit in 2026?» (planning, 6 фото, Нед.1/P0-доверие) + достопримечательность «Butrint National Park» (attractions, 6 фото) + достопримечательность «Albanian Riviera» (attractions, 8 фото) + достопримечательность «Ksamil» (attractions, 6 фото) + статья «Where to Stay in Albania» (planning, хаб where-to-stay, 8 фото, Нед.2) + статья «Albania Travel Insurance Guide (2026)» (insurance, head-термин под спрос GSC, 5 фото, Нед.14 досрочно) + **раздел «Еда» `/food/` — 3 ресторана Тираны** (Oda/Mullixhiu/Komiteti, коллекция `restaurants`); контент-план идёт |
...
### 2026-06-28 — KALENDAR навёл порядок: published на реальных датах, будущее=to-write, baseline-замер, +2 маршрута/Вс
- **Только календарь** (`KALENDAR.md`), src/ не тронут. Якорь 24.06.2026=среда без изменений. Чистка «каши» после оборванной попытки + добавлены воскресные маршруты.
- **Опубликованное сегодня — на дату 28.06, не на будущие слоты.** Проверено: статьи с `publishedAt: 2026-06-28` (`where-to-stay-in-albania`, `albania-travel-insurance`) уже стоят `[x] ОПУБЛИКОВАНО 2026-06-28` на строке **Вс 28.06**, НЕ на будних слотах 29.06/30.06. Перенос не потребовался (0 строк двигать) — они уже на верной дате.
- **0 готовых статей на будущих датах (29.06+) из to-write бэклога.** Единственные `✅` на будущих датах — 4 реально опубликованных live-файла `draft:false` (Saranda 01.07, road-trip 08.07, Berat 10.07, Gjirokaster 31.07): закреплены на своих датах, НЕ переносятся (иначе календарь разойдётся с реальностью; /work их не пересоздаёт). Весь остальной будущий бэклог — ○ к написанию.
- **Будни 29.06–03.08 = ровно 3 контент-слота/день** (статья+маршрут): дни с 1 опубликованным `✅` добраны 2 темами до 3 — структура уже корректна, не менялась.
- **Baseline-замер GA/GSC выполнен 28.06** (сегодня): строка Пн 29.06 `📊 Замер GA/GSC (15 мин)` → `✅ Замер GA/GSC — выполнен 28.06 (baseline)`. Будущие замеры (06.07/13.07/20.07/27.07/03.08) — ○.
- **Маршруты по Вс — по 2 на воскресенье** с **Вс 05.07** (12.07, 19.07, 26.07, 02.08): Albanian Riviera road trip, Albania 4-day, Albania 2-week, Lake Ohrid AL+MK, Albania+Montenegro, Saranda–Corfu combo, Balkan Grand Loop AL+MK+ME+HR, coast-to-mountains, Tirana→Saranda road trip, Albania+Kosovo. Источник — балканские петли. Дублей/неполных route-слотов нет (ровно 2/Вс). 28.06 — особое Вс (день публикации), маршрутов не несёт.
- **Валидатор день↔дата:** независимым скриптом по установленному файлу — **0 ошибок** (109 дат-строк сверены с реальным календарём 2026 от якоря 24.06=среда; каждый будний день 29.06–03.08 = 3 слота, каждое Вс 05.07–02.08 = 2 маршрута). Шапка/итоговый раздел синхронизированы (Вс=2 маршрута, baseline-замер done).

### 2026-06-28 — KALENDAR re-pace: 3 статьи/будний день (ускоренное наполнение до ~80)
- **Только календарь** (`KALENDAR.md`), src/ не тронут. Якорь 24.06.2026=среда без изменений. Темп поднят **5/нед → 3 статьи КАЖДЫЙ будний день (Пн–Пт)** — согласовано владельцем 2026-06-28, чтобы быстрее догнать Грузию (~80 статей). **Сб** — еженедельный новостной дайджест (/news), **Вс** — свободно.
- **Пере-датировано 74 неопубликованных темы** по 3/будний день от ближайшей свободной даты **Пн 29.06.2026** по **Пн 03.08.2026** (26 будних дней). Дни с уже опубликованными статьями (01.07/08.07/10.07/31.07 — по 1 ✅) добраны 2 новыми до 3. Последний день (03.08) — ровно 3.
- **Порядок — коммерция-first** (rule 3, по GSC + v2): (1) транспорт A→B 💰 (how-to-get-around, аэропорт-трансфер, Tirana→Saranda/Berat/Durres/Shkoder/Ohrid, паромы Corfu/Italy, Koman) → (2) where-to-stay по локациям 💰 (Tirana/Saranda/Riviera/resorts) → (3) сравнения X vs Y → (4) cost-of-living/релокация (budget, cost of living, digital nomad, visa-free US) → (5) остальной хвост head→mid→long-tail; депри (itinerary-by-bus, esim-remote-work) в самом конце. Темы НЕ выдуманы и НЕ удалены — только пере-датированы; **страховка повторно НЕ добавлялась** (усилена сегодня).
- **Опубликованное не двигалось** (9 строк на реальных датах): Tirana 24.06, is-expensive 25.06, is-safe 26.06, Saranda 01.07, road-trip 08.07, Berat 10.07, Gjirokaster 31.07; **where-to-stay + travel-insurance помечены [x] на реальной дате публикации 28.06** (бывшие плановые слоты 29.06/22.09 освобождены).
- **Валидатор день↔дата:** независимым скриптом по установленному файлу — **0 ошибок** (все строки Пн–Пт сверены с реальным календарём 2026 от якоря 24.06=среда; каждый будний день кроме последнего = ровно 3 статьи; 83 уник. слага = 74 неопубл. + 9 опубл.; дублей нет). Шапка/метаданные обновлены под новый темп; итоговый раздел переписан.

### 2026-06-28 — SEO-PUBLISH: «Albania Travel Insurance Guide (2026)» (insurance, head, спрос GSC)
- **Зачем:** Google Search Console показывает доказанный спрос на head-термин `albania travel insurance` / `travel insurance albania` — 60+ показов, но позиция 64–94 (не ранжируемся). Денежная тема (💰 страховка). Создана отдельная таргет-страница под точный запрос (хаб `/insurance/` и `do-you-need-insurance` его не таргетировали: хаб = регуляторика, do-you-need = вопросный intent).
- **Создано** (`src/content/articles/en/albania-travel-insurance.md`, `draft:false`): URL `/insurance/albania-travel-insurance/`, category `insurance` (head-термин в страховом силосе, рядом с хабом и do-you-need-insurance; self-canonical, без дубля). On-page под запрос: title «Albania Travel Insurance Guide (2026)» (37 симв.), H1 то же, **первый H2 «Do you need travel insurance for Albania?»**, desc 143 симв. — везде точный термин ведёт. ~1548 слов: нужна ли (Албания≠Шенген: нет шенгенского мин. €30k на границе, EHIC/GHIC не действует; ВНЖ/номады — обязательна) → что покрывает (медицина + **эвакуация/репатриация** в приоритете) → активности/исключения (хайкинг/высота/Theth–Valbona, скутер, дайвинг) → сколько стоит (**только диапазоны + «always get a live quote»**, без выдуманных цифр; «нет офиц. минимума для туристов») → short vs long stay → bottom line.
- **YMYL:** видимый blockquote-дисклеймер сразу после лида («general information, not insurance or medical advice — verify with official source, figures are ranges, get a current quote»). Условия — со ссылкой на gov.uk/travel.state.gov/smartraveller (`sources`). Цены не выдуманы.
- **Фото — 5** (CC, Wikimedia, webp ≤200КБ, автор+лицензия+sourceUrl на Commons; темы = страховые сценарии): cover Albanian Alps/Theth (Anna Ilieva-Alikaj, CC BY-SA 4.0); инлайн mountains in Theth + Theth river (Alexkom000, CC BY 4.0), Albania coast aerial (kallerna, CC BY-SA 4.0); gallery Ksamil (ImogenX, CC BY 2.0) + Theth river. Лицензии сверены по Commons API.
- **Перелинк — 9 внутр. ссылок** (формат `/{category}/{slug}/`): ← is-albania-safe, ← albania-visa, ← is-albania-expensive (бюджет), ← albanian-riviera, ← digital-nomad-visa, ← how-to-rent-a-car, ← best-time-to-visit, ↔ do-you-need-insurance, → /insurance/ hub. 2× AffiliateBox safetywing через `/go/safetywing?c=albania-travel-insurance`.
- **Гейты:** `npm run qa` → **GO** (critical 0 · medium 0; минорные 54 — только pre-existing decorative-alt на главной/404). check 0/0/0 · build (39 стр) · test (enums+parity+photos+interlinks) ✓ · test:links ✓ · lint ✓. KALENDAR Нед.14 Вт 22.09 отмечен `[x]` (опубликовано досрочно 2026-06-28).
