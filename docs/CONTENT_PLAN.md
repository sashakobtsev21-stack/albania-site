> ⚠️ **СПРАВОЧНИК кластеров/слагов — НЕ активный план. СУПЕРСЕДЕД 2026-06-22.**
> Актуальный по-дневной контент-план и трекер «что опубликовано / в очереди» — **только в `/KALENDAR.md`** (построен по SEO-аудиту). Этот файл оставлен исключительно как **тематическая справка**: карта EN-интентов/слагов по кластерам + чек-лист YMUL-источников «Сверять перед публикацией». **Статусы тем здесь не поддерживаются и могут не отражать реальность** (на сайте уже ~25 статей — см. KALENDAR/PROGRESS). НЕ редактировать как план и не сверяться по нему о состоянии публикаций.

# CONTENT_PLAN.md — справочник кластеров и слагов (Albania)

Справка: какие кластеры/темы существуют, под какой EN-запрос и с каким слагом их писать.
Правила написания — `CONTENT_GUIDE.md`; продукт/ниша — `SPEC.md` → `CLAUDE.md`; стратегия/приоритеты —
`STRATEGY.md`; **что и когда публикуем — `KALENDAR.md`** (единственный источник правды по плану/статусу).

- **Колонки `Status` ниже — историческое, НЕ актуально.** За реальным состоянием темы — в `KALENDAR.md`.
- **Сайт одноязычный — только en** (ru/uk удалены 2026-06-22). URL: `/<category>/<slug>/`.
...
- **Гео/язык:** **Tier-1/Запад через EN** (премиум-RPM), сайт одноязычный en (STRATEGY §2; ru/uk удалены 2026-06-22).
  Ключи и темы — от **EN-интента** (как ищет западный турист/номад).
...
> **Очередь публикаций (что писать первым) — перенесена в `KALENDAR.md`.**
> Раньше здесь был пилотный список §2; он дублировал план и протух. Порядок и приоритет тем — только в по-дневном `KALENDAR.md` (сортировка по реальному EN-спросу из GSC).

---

## 3. Правила ведения
1. **Приоритет кластеров** — как в §0 (Планирование → MONEY → Релокация → Маршруты → Достопримечательности → Города → Еда → Развлечения).
2. **EN-only:** одна en-версия от EN-интента (ru/uk удалены 2026-06-22, сайт одноязычный); пишем по `CONTENT_GUIDE.md`. Перед публикацией — `npm run qa` = **GO**.
...
- **EN (единственный язык):** Google autocomplete + «People also ask» + Keyword Planner; запрос носителя, не калька с русского. Интент: транзакционный (rent/insurance/book) → MONEY; справочный (what to see/how to get) → TRAFFIC.
