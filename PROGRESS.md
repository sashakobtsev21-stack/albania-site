...
| R3 Контент | 🟡 | 5 статей (cover-only) + столица Тирана (18 фото, Нед.1) + город Саранда (14 фото, Нед.2) + город Берат (12 фото, Нед.3) + МАРШРУТ Albania Road Trip (6 остановок, 10 фото, Нед.3) + статья «Is Albania Safe to Visit in 2026?» (planning, 6 фото, Нед.1/P0-доверие) + достопримечательность «Butrint National Park» (attractions, 6 фото) + достопримечательность «Albanian Riviera» (attractions, 8 фото) + **раздел «Еда» `/food/` — 3 ресторана Тираны** (Oda/Mullixhiu/Komiteti, коллекция `restaurants`); контент-план идёт |
